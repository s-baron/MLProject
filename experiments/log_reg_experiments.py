...
ngram_range = [("ngram_range", (1,x)) for x in range(1,2)]
stop_wordsList = [("stop_words",x) for x in ["english", None]]
max_features = [("max_features", x) for x in [None, 5000]]
norm = [("norm", x) for x in ["l2", None]]
allParams = [vectorizer, ngram_range, stop_wordsList, max_features, norm]
